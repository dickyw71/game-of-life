...
it('should produce a matrix of 50x30 elements, all elements have a value of there position in the matrix', () => {
    var a = new Array(30);
    for (let i = 0; i < 30; i++) {
        a[i] = new Array(50);
        for (let j = 0; j < 50; j++) {
   ...
   expect(a.length).toEqual(30);
   for(let ix =0; ix<a.length; ix++) {
    expect(a[ix][ix].x).toEqual(ix);
    expect(a[ix][ix].y).toEqual(ix);
   }

   expect(a[12][29].x).toEqual(12);
   expect(a[12][29].y).toEqual(29);
})
